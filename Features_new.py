...
class ContinuousFeature():
    ...
    def __generate_bins(self, bins_num):
        df = self.df[[self.feature, 'Decision']]
        ...
        for bins_num in range(bins_max, 1, -1):
            ...
            df_grouped = pd.DataFrame(df.groupby('bin') \
                                      .agg({self.feature: 'count',
                                            'Decision': 'sum'})) \
                                      .reset_index()
            r, p = stats.stats.spearmanr(df_grouped['bin'], df_grouped['Decision'])
            ...
            if (
                    ...
                    and not (df_grouped[self.feature] == df_grouped['Decision']).any()   # check if number of good and bad is not equal to 0
            ):
                break
